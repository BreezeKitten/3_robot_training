...
resTH = PI/15
LOG_DIR = 'logs/Residual_Learning'
...
def Predict_action_value(agent1, agent2, agent3, V_pred, W_pred):
    ...
    action_value = R + value_matrix[0][0] + nearest_value
...
def Divide_state_value(data):
    ...
    return state, value, residual_value
...
def DL_process(DL_database):
    ...
    for training_eposide in range(training_eposide_num):
        ...
        training_state, training_value, training_residual_value = Divide_state_value(training_data)
        ...
        if training_eposide%1000 == 0:
            two_to_three_network_saver.save(sess,'Residual_network/test.ckpt')   
...
if __name__ == '__main__':
    '''
    NOW = datetime.datetime.now().strftime('%Y%m%d-%H%M%S') + 'Residual_init'
    ...
    two_to_three_network_saver.restore(sess,'Residual_network/test.ckpt')
    

    LAST_SAVE_PATH = 0
    for i in range(20):
        
        NOW = datetime.datetime.now().strftime('%Y%m%d-%H%M%S') + '_Residual_RL'
        FM = file_manger.file_manger(LOG_DIR,NOW)
        SAVE_DIR = FM.log_path
        FM.create_dir()    
