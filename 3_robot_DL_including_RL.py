...
def Predict_action_value(agent1, agent2, agent3, V_pred, W_pred):
    ...
    value_matrix12 = sess_2_robot.run(mid_value, feed_dict={state: state_pred12})
    ...
    value_matrix13 = sess_2_robot.run(mid_value, feed_dict={state: state_pred13})
    ...
    mid_state_pred = [[action_value12, action_value13, D12, D13]]
    value_matrix = sess_3_robot.run(predict_value, feed_dict={mid_state: mid_state_pred})
...
def DL_process(DL_database):
    ...
    saver.save(sess_3_robot,'2_to_3_robot_network/test.ckpt')    
    return
...
if __name__ == '__main__':
    ...
    NOW = datetime.datetime.now().strftime('%Y%m%d-%H%M%S') + '_3_robot'
    ...
    predict_value, mWf, mBf = add_layer(mH2, m_layer2_output_number, 1, 'mWf', 'mBf', activation_function=tf.nn.sigmoid)
    ...
    saver.restore(sess_3_robot,'2_to_3_robot_network/test.ckpt')
